  ...
  var pageFunction = function () {
    ...
    var loadDatatables = function () {
      ...
      $dt = $dtBasic.DataTable({
        ...
        ajax: {
          url: $.helper.baseApiPath("/fundamental_analysis/getDatatables"),
          type: "POST",
        ...
        columns: [
          ...
          // },
          {
            ...
            render: function (data, type, row) {
              ...
              var html = "";
              if (type === 'display') {
                html = `<a href="/admin/fundamental_analysis/detail?id=` + row.id + `" class="font-weight-bold" style="font-size: 10pt;">` + data + `</a>`;
              }
              return html;
            }
          },
          ...
          // },
          {
            ...
            class: "text-left",
            render: function (data, type, row) {
              var html = thousandSeparatorInteger(row.current_price);
              if (type === 'display') {
              }
              return html;
            }
          },
          {
            ...
            class: "text-left",
            render: function (data, type, row) {
              var html = thousandSeparatorInteger(row.normal_price);
              if (type === 'display') {
              }
              return html;
            }
          ...
          },
          {
            ...
            render: function (data, type, row) {
              ...
              if (type == 'display') {
                ...
                html += `<a href="/admin/fundamental_analysis/detail?id=` + row.id + `" type="button" class="btn btn-primary btn-xs font-weight-bold d-sm-inline-block shadow-md mr-1" data-id="` + data + `" data-name="` + row.name + `" style="min-width: 50px;">Ubah</a>`;
    ...
    var deleteById = function (id, name) {
      Swal.fire({
        ...
        if (result.value) {
          $.ajax({
            url: $.helper.baseApiPath("/fundamental_analysis/deleteById/" + id),
            type: 'DELETE',
            success: function (r) {
              console.log(r);
              if (r.status) {
