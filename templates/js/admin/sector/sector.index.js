  ...
  var pageFunction = function () {
    ...
    var loadDatatables = function () {
      ...
      $dt = $dtBasic.DataTable({
        ...
        ajax: {
          url: $.helper.baseApiPath("/sector/getDatatables"),
          type: "POST",
    ...
    var SaveOrUpdate = function (e) {
      ...
      if (isvalidate) {
        ...
        console.log(record);
        $.ajax({
          url: $.helper.baseApiPath("/sector/save"),
          type: 'POST',
          data: record,
          success: function (r) {
            console.log(r);
            if (r.status) {
              ...
              var $message = "Berhasil menambah Sector";
              if (record.id != "") $message = "Berhasil mengubah Sector";
              ...
              $.each(r.errors, function (index, value) {
                ...
                if (value.includes(`unique constraint "uk_name"`) || value.includes("kunci ganda")) {
                  toastr.error(record.name + " sudah terdaftar. Silahkan cek kembali daftar sector.", 'Peringatan!');
                } else {
    ...
    var getById = function (id) {
      $.ajax({
        url: $.helper.baseApiPath("/sector/getById/" + id),
        type: 'GET',
        success: function (r) {
          if (r.status) {
    ...
    var deleteById = function (id, name) {
      Swal.fire({
        ...
        if (result.value) {
          $.ajax({
            url: $.helper.baseApiPath("/sector/deleteById/" + id),
            type: 'DELETE',
            success: function (r) {
              console.log(r);
              if (r.status) {
