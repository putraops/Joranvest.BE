...
func (c Client) CardToken(card midtrans.CardDetails, clientKey string) (*CardTokenResponse, *midtrans.Error) {
	resp := &CardTokenResponse{}
	URL := c.Env.BaseUrl() +
		"/v2/token?client_key=" + clientKey +
		"&card_number=" + card.CardNumber +
		"&card_exp_month=" + strconv.Itoa(int(card.ExpMonth)) +
		"&card_exp_year=" + strconv.Itoa(int(card.ExpYear)) +
		"&card_cvv=" + strconv.Itoa(int(card.CVV))
...
// CardToken : Do `/token` API request to Midtrans Core API return `coreapi.CardTokenResponse`,
// more detail refer to: https://api-docs.midtrans.com/#get-token
func CardToken(card midtrans.CardDetails) (*CardTokenResponse, *midtrans.Error) {
	c := getDefaultClient()
	return c.CardToken(card, c.ClientKey)
}
