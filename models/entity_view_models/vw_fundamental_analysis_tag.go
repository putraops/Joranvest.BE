...
type EntityFundamentalAnalysisTagView struct {
...
func (EntityFundamentalAnalysisTagView) TableName() string {
	return "vw_fundamental_analysis_tag"
}

func (EntityFundamentalAnalysisTagView) ViewModel() string {
	...
	sql.WriteString("  r.fundamental_analysis_id,")
	...
	sql.WriteString(" FROM fundamental_analysis_tag r ")
	sql.WriteString("  LEFT JOIN fundamental_analysis f ON f.id = r.fundamental_analysis_id")
	sql.WriteString("  LEFT JOIN tag t ON t.id = r.tag_id")
	sql.WriteString("  LEFT JOIN application_user u1 ON u1.id = r.created_by")
	sql.WriteString("  LEFT JOIN application_user u2 ON u2.id = r.updated_by")
	return sql.String()
}
func (EntityFundamentalAnalysisTagView) Migration() map[string]string {
	var view = EntityFundamentalAnalysisTagView{}
