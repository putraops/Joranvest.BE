...
func (db *webinarSpeakerConnection) GetSpeakersRatingByWebinarId(webinarId string) helper.Response {
	...
	sql.WriteString("	o.filepath AS organization_filepath,")
	sql.WriteString("	o.filepath_thumbnail AS organization_filepath_thumb,")
	sql.WriteString("	concat(u3.first_name, ' ', u3.last_name) AS speaker_fullname, ")
	sql.WriteString("	u3.filepath AS speaker_filepath, ")
	sql.WriteString("	u3.filepath_thumbnail AS speaker_filepath_thumb ")
